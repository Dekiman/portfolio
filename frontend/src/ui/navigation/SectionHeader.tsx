import type { CSSProperties, ReactNode } from "react";
import { memo, useEffect, useRef } from "react";
import { HeaderIcon, type HeaderIconKind } from "./HeaderIcon";
import { usePrefersReducedMotion } from "../../hooks/usePrefersReducedMotion";

type SectionHeaderProps = {
  sectionNames: string[];
  activeIndex: number;
  onSectionClick: (index: number) => void;
  children?: ReactNode;
  className?: string;
  compact?: boolean;
};

const DISPLACEMENT_TEXTURE =
  "data:image/png;base64,";

const SECTION_ICONS: Record<string, HeaderIconKind> = {
  home: "home",
  about: "about",
  projects: "projects",
  experience: "experience",
  contact: "contact",
};

function getSectionIcon(sectionName: string): HeaderIconKind {
  return SECTION_ICONS[sectionName.toLowerCase()] ?? "home";
}

export const SectionHeader = memo(function SectionHeader({
  sectionNames,
  activeIndex,
  onSectionClick,
  children,
  className,
  compact = false,
}: SectionHeaderProps) {
  const reduceMotion = usePrefersReducedMotion();
  const navRef = useRef<HTMLElement | null>(null);
  const indicatorRef = useRef<HTMLSpanElement | null>(null);
  const labelRefs = useRef<Array<HTMLSpanElement | null>>([]);
  const itemCount = sectionNames.length;
  const safeIndex = Math.min(
    Math.max(activeIndex, 0),
    Math.max(itemCount - 1, 0),
  );

  useEffect(() => {
    const navNode = navRef.current;
    const indicatorNode = indicatorRef.current;
    const activeLabel = labelRefs.current[safeIndex];
    if (!navNode || !indicatorNode || !activeLabel) {
      return undefined;
    }

    const applyIndicatorMetrics = () => {
      const navRect = navNode.getBoundingClientRect();
      const labelRect = activeLabel.getBoundingClientRect();
      // Absolute positioning is resolved from the nav padding box, not its border box.
      const indicatorLeft =
        labelRect.left - navRect.left - navNode.clientLeft;
      const indicatorTop =
        labelRect.top - navRect.top - navNode.clientTop;

      indicatorNode.style.width = `${labelRect.width}px`;
      indicatorNode.style.height = `${labelRect.height}px`;
      indicatorNode.style.transform = `translate3d(${indicatorLeft}px, ${indicatorTop}px, 0)`;
      indicatorNode.style.opacity = "1";
    };

    const syncOnNextFrame = () => {
      window.requestAnimationFrame(applyIndicatorMetrics);
    };

    let resizeObserver: ResizeObserver | null = null;
    if (typeof ResizeObserver !== "undefined") {
      resizeObserver = new ResizeObserver(syncOnNextFrame);
      resizeObserver.observe(navNode);
      resizeObserver.observe(activeLabel);
    }

    applyIndicatorMetrics();
    const fonts = document.fonts;
    fonts?.addEventListener?.("loadingdone", syncOnNextFrame);
    window.addEventListener("resize", syncOnNextFrame);

    return () => {
      resizeObserver?.disconnect();
      fonts?.removeEventListener?.("loadingdone", syncOnNextFrame);
      window.removeEventListener("resize", syncOnNextFrame);
    };
  }, [compact, safeIndex, sectionNames.length]);

  const indicatorStyle = {
    opacity: 0,
    "--floating-indicator-duration": reduceMotion ? "0ms" : "420ms",
  } as CSSProperties;

  const sectionHeaderClassName = `floating-sections section-header-card${
    compact ? " section-header-card--compact" : ""
  }${className ? ` ${className}` : ""}`;

  const filterDefinition = (
    <svg
      width="0"
      height="0"
      style={{ position: "absolute", pointerEvents: "none" }}
      aria-hidden="true"
    >
      <filter id="displacementFilter">
        <feImage
          href={DISPLACEMENT_TEXTURE}
          preserveAspectRatio="none"
          result="turbulence"
        />
        <feDisplacementMap
          in="SourceGraphic"
          in2="turbulence"
          scale={200}
          xChannelSelector="R"
          yChannelSelector="B"
        />
      </filter>
    </svg>
  );

  if (compact) {
    return (
      <nav
        ref={navRef}
        className={sectionHeaderClassName}
        aria-label="Portfolio sections"
      >
        {filterDefinition}
        <span
          ref={indicatorRef}
          className="floating-sections__indicator floating-sections__indicator--compact"
          style={indicatorStyle}
          aria-hidden="true"
        >
          <span className="floating-sections__indicator-glass" />
        </span>
        {sectionNames.map((sectionName, index) => (
          <button
            type="button"
            key={sectionName}
            className={`floating-sections__item floating-sections__item--compact${
              index === safeIndex ? " floating-sections__item--active" : ""
            }`}
            aria-current={index === safeIndex ? "location" : undefined}
            aria-label={`Go to ${sectionName}`}
            onClick={() => onSectionClick(index)}
          >
            <span
              className="floating-sections__label floating-sections__label--icon-only"
              ref={(node) => {
                labelRefs.current[index] = node;
              }}
              aria-hidden="true"
            >
              <span className="floating-sections__icon-shell">
                <HeaderIcon
                  kind={getSectionIcon(sectionName)}
                  className="floating-sections__icon"
                />
              </span>
            </span>
          </button>
        ))}
        {children}
      </nav>
    );
  }

  return (
    <nav
      ref={navRef}
      className={sectionHeaderClassName}
      aria-label="Portfolio sections"
    >
      {filterDefinition}
      <span
        ref={indicatorRef}
        className="floating-sections__indicator"
        style={indicatorStyle}
        aria-hidden="true"
      >
        <span className="floating-sections__indicator-glass" />
      </span>
      {sectionNames.map((sectionName, index) => (
        <button
          type="button"
          key={sectionName}
          className={`floating-sections__item ${index === safeIndex ? "floating-sections__item--active" : ""}`}
          aria-current={index === safeIndex ? "location" : undefined}
          aria-label={`Go to ${sectionName}`}
          onClick={() => onSectionClick(index)}
        >
          <span
            className="floating-sections__label"
            ref={(node) => {
              labelRefs.current[index] = node;
            }}
          >
            <span className="floating-sections__label-text">{sectionName}</span>
          </span>
        </button>
      ))}
      {children}
    </nav>
  );
});
