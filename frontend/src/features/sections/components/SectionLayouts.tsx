...
import type { FixedSection, SectionCard, SectionLayout } from "../types/section";
...
function toPopupCardId(sectionId: string, title: string): string {
  return `${sectionId}-${title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "")}`;
}
...
function ProjectsLayout({ section }: SectionLayoutRendererProps) {
  const projectCards: PopupCardItem[] = (section.cards ?? []).map((card, index) => ({
  ...
  return (
    <div className="projects-layout">
      ...
      <PopupCards items={projectCards} ariaLabel={`${section.title} projects`} />
